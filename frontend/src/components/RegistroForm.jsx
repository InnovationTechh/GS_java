  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setToast({
        open: true,
        type: "success",
        message: edit ? "Registro atualizado com sucesso!" : "Registro cadastrado com sucesso!"
      });

      const novoId = res?.id ?? id;
      setTimeout(() => nav(`/registros/editar/${novoId}`), 900);
    } catch (err) {
